class MathUtilsCtor {
  ...
  roundTo(x: number, precision: number): number {
    const shift = (x: number, precision: number, reverseShift: boolean = false) => {
      if (reverseShift) {
        precision = -precision
      }
  ...
  roundToDigitCount(x: number, digitCount: number = 10): number {
...
export const MathUtils = new MathUtilsCtor()
